...
@Suite struct PortScannerScanJoinTests {
    ...
    @Test func agentRootPIDsUnionsAcrossWorkspaces() {
        let other = UUID()
        let roots = PortScanner.agentRootPIDs(in: [
            workspaceId: [
                AgentPortRootIdentity(pid: 100, processIdentity: nil),
                AgentPortRootIdentity(pid: 101, processIdentity: nil),
            ],
            other: [AgentPortRootIdentity(pid: 200, processIdentity: nil)],
        ])
