...
public final class UpdateController {
    ...
    /// When `true`, all update activity is suppressed: the updater never starts, no launch
    /// probe runs (so the passive "update available" indicator never appears), and explicit
    /// checks no-op. Injected by the host for builds whose `SUFeedURL` would offer the wrong
    /// artifact — e.g. a STAGING build, whose version trails the production appcast it points
    /// at and which would clobber its isolated bundle if "updated".
    ...
    ///   - updatesDisabled: When `true`, suppresses all update activity (no updater start,
    ///     no launch probe, no-op explicit checks). Defaults to `false`. The host passes
    ///     `true` for builds that must not self-update, such as STAGING.
    ...
    private func checkForUpdatesWhenReady() {
        guard !updatesDisabled else {
            log.append("check for updates ignored (host build)")
            model.setState(.idle)
            return
        }
    ...
    public func startUpdaterIfNeeded() {
        guard !updatesDisabled else {
            log.append("updates disabled (host build); updater not started")
            return
        }
