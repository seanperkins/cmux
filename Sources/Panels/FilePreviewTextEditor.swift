...
struct HighlightedFilePreviewRouter: View {
    ...
    /// Whether long lines soft-wrap, from the persisted `fileEditor.wordWrap`
    /// setting. Forwarded to whichever editor is chosen so the toggle behaves
    /// identically for highlighted and plain previews.
    let wordWrap: Bool
