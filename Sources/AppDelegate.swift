...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation, NSMenuDelegate, CmuxConfigStoreReloadEnvironment {
    ...
    private lazy var updateController = UpdateController(
        log: updateLog,
        // STAGING points at the production appcast; suppress self-update so it neither shows a
        // spurious "update available" indicator nor clobbers its isolated bundle.
        updatesDisabled: SocketControlSettings.isStagingBundleIdentifier(Bundle.main.bundleIdentifier)
    )
