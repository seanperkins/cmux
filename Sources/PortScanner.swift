...
final class PortScanner: @unchecked Sendable {
    ...
    private func finishScan(
        ...
        let join = Self.joinScanResults(
            ...
            pidToTTY: validPIDToTTY,
            agentOwnershipByPID: agentOwnershipByPID,
            agentRootPIDs: Self.agentRootPIDs(in: agentRootsByWorkspace)
        )
        ...
        let agentPortsSnapshot = join.agentPortsByWorkspace
    ...
    /// Output of ``joinScanResults(pidToPorts:pidToTTY:agentOwnershipByPID:agentRootPIDs:)``.
    ...
    static func joinScanResults(
        ...
        pidToTTY: [Int: String],
        agentOwnershipByPID: [Int: Set<UUID>],
        ...
        for (pid, ports) in pidToPorts {
            guard !agentRootPIDs.contains(pid),
                  let ownership = agentOwnershipByPID[pid] else { continue }
